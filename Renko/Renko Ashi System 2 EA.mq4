...
extern int stoplevel=15;

extern string entrylogics="Entry Logics";

extern bool changedirection=false;
extern bool machannelsignal=true;
extern int maperiod=5;
extern bool mainappliedpricefilter=true;
extern int MAPeriod=10;
extern int MAType=3;
extern int MAAppliedPrice=6;
extern bool smc4scalperfilter=true;
extern bool macdfilter=true;
...
*/
   ...
   if(time!=Time[0] || shift==0){
      double mau=iMA(NULL,0,maperiod,0,MODE_SMMA,PRICE_HIGH,shift);
      double mad=iMA(NULL,0,maperiod,0,MODE_SMMA,PRICE_LOW,shift);
      double maua=iMA(NULL,0,maperiod,0,MODE_SMMA,PRICE_HIGH,shift+1);
      double mada=iMA(NULL,0,maperiod,0,MODE_SMMA,PRICE_LOW,shift+1);
   
      double smu=iCustom(NULL,0,"smC4Scalper Candles_v3.1",3,shift);
      double smd=iCustom(NULL,0,"smC4Scalper Candles_v3.1",2,shift);
      
      double macu=iCustom(NULL,0,"MA in Color_wAppliedPrice",MAPeriod,MAType,MAAppliedPrice,1,shift);
      double macd=iCustom(NULL,0,"MA in Color_wAppliedPrice",MAPeriod,MAType,MAAppliedPrice,2,shift);
      double macua=iCustom(NULL,0,"MA in Color_wAppliedPrice",MAPeriod,MAType,MAAppliedPrice,1,shift+1);
      double macda=iCustom(NULL,0,"MA in Color_wAppliedPrice",MAPeriod,MAType,MAAppliedPrice,2,shift+1);
      
      double macdi=iCustom(NULL,0,"MACD Alert",FastEMA,SlowEMA,SignalSMA,0,shift);
      double macdia=iCustom(NULL,0,"MACD Alert",FastEMA,SlowEMA,SignalSMA,0,shift+1);
      double macdiu=iCustom(NULL,0,"MACD Alert",FastEMA,SlowEMA,SignalSMA,2,shift);
      double macdid=iCustom(NULL,0,"MACD Alert",FastEMA,SlowEMA,SignalSMA,3,shift);
      time=Time[0];
   }
   bool buy=false;
   bool sell=false;
   ...
   if(
   (smc4scalperfilter==false || (smc4scalperfilter && smu>smd))
   && (mainappliedpricefilter==false || (mainappliedpricefilter && macu>macua && macu!=EMPTY_VALUE))
   && (machannelsignal==false || (machannelsignal && Open[shift+1]<maua && Open[shift]>mau))
   && (macdfilter==false || (macdfilter && macdi>macdia && macdiu==macdi))
   && (changedirection==false || (changedirection && continuebuy))
   ){
      if(reversesignals)sell=true;else buy=true;
      continuebuy=false;
      continuesell=true;
      if(autosl){stoploss=(Ask-mad)/pt;if(stoploss<stoplevel)stoploss=stoplevel;}
   }
   
   if(
   (smc4scalperfilter==false || (smc4scalperfilter && smu<smd))
   && (mainappliedpricefilter==false || (mainappliedpricefilter && macd<macda && macda!=EMPTY_VALUE))
   && (machannelsignal==false || (machannelsignal && Open[shift+1]>mada && Open[shift]<mad))
   && (macdfilter==false || (macdfilter && macdi<macdia && macdid==macdi))
   && (changedirection==false || (changedirection && continuesell))
   ){
      if(reversesignals)buy=true;else sell=true;
      continuebuy=true;
      continuesell=false;
      if(autosl){stoploss=(mau-Ask)/pt;if(stoploss<stoplevel)stoploss=stoplevel;}
   }
...
int open(int type,double lots,double price,double stoploss,double takeprofit,int expire,color clr){
   ...
   ticket=OrderSend(Symbol(),type,NormalizeDouble(lots,lotdigits),NormalizeDouble(price,dg),slippage*mt,sl,tp,comment+" "+DoubleToStr(magic,0),magic,expire,clr);
   return(ticket);
}
