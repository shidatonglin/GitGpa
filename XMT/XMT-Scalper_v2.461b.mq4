...
//---------------------------------------------------------------------------------------------------
// Ver 2.461b - 2015-10-05 by Robin Hood
// - Added Time Filter
// - Added Points Movement Filter (on last tick)
// - Changed Trailing (no more trailing on TP)
//---------------------------------------------------------------------------------------------------
...
//#include <sdlib.mqh>        // "stdlib.mqh" or "<sdlib.mqh> 
...
extern int 		MaxExecution 					= 700; // Max allowed average execution time in ms (0 means no restrictions)
extern int 		MaxExecutionMinutes 			= 5; // How often in minutes should fake orders be sent to measure execution speed
extern double 	StopLoss 						= 20;	// StopLoss from as many points. Default 60 (= 6 pips)
extern double 	TakeProfit 						= 200; // TakeProfit from as many points. Default 100 (= 10 pip)
extern double 	AddPriceGap 					= 5; // Additional price gap in points added to SL and TP in order to avoid Error 130
extern double 	TrailingStart 					= 20;	// Start trailing profit from as so many points. 
extern double 	Commission 						= 4; // Some broker accounts charge commission in USD per 1.0 lot. Commission in dollar
...
// ADDED BY ROBIN HOOD
extern string TimeFilter = "===== Time Filters ===== ";
...
extern string PointsMovement = "===== Points per Tick Filter ===== ";

extern bool MaximumMoveEnable = true;  
extern int  MaximumMovePerTick  = 4;
...
string EA_version = "XMT-Scalper v. 2.461 - Time Filter / Tick Speed Filter";
...
bool TradeSession(){
   ...
   if((UseFridayFinalTradeTime && (Hour()>=FridayFinalHourGMT+ServerTimeZone) && DayOfWeek()==5) || DayOfWeek()==0) return(0); // Friday Control
   if((TimeControl(HourStartTrade,HourStopTrade)!=1 && TimeControl && HourStartTrade<HourStopTrade)
      ...
      || !TimeControl)return(1); // "Trading Time";
  return(0); // "Non-Trading Time";
 }
  


//+------------------------------------------------------------------+
// Points are moving too fast?
//+------------------------------------------------------------------+

bool MovingTooFast(){
    if ( PointsMov > MaximumMovePerTick ) 
    {
    return(1);
    Alert ("Points moving too fast!");
    }
	  
    if ( PointsMov  <= MaximumMovePerTick ) return(0);
    return(0);
    }
...
//|                                                                  |
//+------------------------------------------------------------------+
string MinToHours(int minutes)
  {
   int hours = minutes/60;
   int mins  = minutes%60;
   return("hrs="+hours+" mins="+mins);//+" (minutes: "+minutes+")");
  }
...
//+------------------------------------------------------------------+
// Price movements in points per tick.
//+------------------------------------------------------------------+

	// Points moves in each tick
	int price=MathRound(Bid/Point),dif;
   if (price_prev==price) PointsMov = 0;
   dif=MathAbs(price-price_prev);
   string par = DoubleToStr(dif,0);
   PointsMov = par;
   price_prev=price;
...
void sub_trade() 
{
 ...
 //==========================================================// 
			...
			case OP_BUY:
				...
				while ( TRUE )  
				{
					...
					if ( ordertakeprofit < sub_normalizebrokerdigits ( askpluscommission + TakeProfit * Point + AddPriceGap ) && askpluscommission + TakeProfit * Point + AddPriceGap - ordertakeprofit > TrailingStart ) 
					{
						...
						// ROBIN HOOD: No more TP Trailing
						...
						if ( orderstoploss != OrderStopLoss()  ) //  && ordertakeprofit != OrderTakeProfit()  (ROBIN HOOD: No more TP Trailing)
						{
							// Start Execution timer
							Execution = GetTickCount();
							// Try to modify order
							wasordermodified = OrderModify ( OrderTicket(), 0, orderstoploss, orderexpiretime, Lime ); // ordertakeprofit, (ROBIN HOOD: No more TP Trailing)
						}
				...
				while ( TRUE ) 
				{
					...
					{					
						...
						if ( orderstoploss != OrderStopLoss() ) // && ordertakeprofit != OrderTakeProfit() (ROBIN HOOD: No more TP Trailing)
						{
							// Start Execution timer
							Execution = GetTickCount(); 
							wasordermodified = OrderModify ( OrderTicket(), 0, orderstoploss,  orderexpiretime, Orange ); //ordertakeprofit, (ROBIN HOOD: No more TP Trailing)
						}
	...
	//TickVolume
	//int TicksVolume = iVolume(NULL,PERIOD_M1,1);
	...
	// ROBIN HOOD: Before executing new orders, lets check the Trading Session.
	if ( pricedirection != 0 && TimeControl == true && TradeSession() == FALSE ) 
	{   
		pricedirection = 0; // Ignore the order opening triger
		if ( Debug || Verbose )
			Print ( " Trading Session is not started. No permission to trade. " );
	}
	
	
   //ROBIN HOOD: Before executing new orders, check if Points are not moving too fast. 
   
	if ( pricedirection != 0 && MaximumMoveEnable == true && MovingTooFast() ) 
	{   
		pricedirection = 0; // Ignore the order opening triger
		if ( Debug || Verbose )
			//Alert ( " Prices moving to fast! Trade stopped." );
			Print ( " Prices moving to fast! Trade stopped." );
	}
	...
	else 
	{
		...
		else 
		{
			...
			if ( Debug || Verbose ) 
			{
				...
				// ADDED BY ROBIN	HOOD
					textstring = textstring + "\n" + "Points per Tick: " +  PointsMov ;// 
					textstring = textstring + "\n" + "Moving too Fast:  " + MovingTooFast(); // + MovingTooFast()
				...
				if ( MaxExecution > 0 ) 
				{
					textstring = textstring + "\n" + "The current Avg Execution is (" + Avg_execution +" ms) while you have set as MaxExecution (" 
					+ MaxExecution+ " ms)";
				}
				Comment ( textstring );
...
//	linespace = textspacing * 2 + Text_Size * 9 + 3 * 9 + 40;		
//	sub_Display ( "line11", line11, Text_Size, 3, linespace, Color_Section1, 0 ); 
