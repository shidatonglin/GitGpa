...
extern string TITLE = "OVER NIGHT EA V9a";
extern string DETAILS = "Magic Number & Trade Comment";
extern int iMagicNumber = 77777108;
extern string trade_comment="OVERNIGHT V9a";
int     iMagicNumber2;
datetime Time0 = 0;
extern string MAIN_SETTINGS = "TP1, TP2, SL and TSL settings.";
extern int TakeProfit1 = 22;
extern int TakeProfit2 =50;
extern int StopLoss = 140;
double Poin;
extern bool   useTrailing = true;
extern double iTrailingStop = 22;
...
extern int exit_time=7;
extern int exit_pips=8;
   ...
   for (int j = 0; j < OrdersTotal(); j++) {
         ...
         if (OrderType() == OP_BUY && (OrderMagicNumber() == iMagicNumber)) {
            if (Bid < (OrderOpenPrice()-StopLoss*Poin) 
             ...
             || (TimeCurrent() > OrderOpenTime()+ (exit_time*3600)&& Bid >=OrderOpenPrice()+(exit_pips*Poin))
             ) {
               RefreshRates();
               OrderClose(OrderTicket(), OrderLots(), Bid, 3*Poin/Point, Aqua);
               }
               }
          if (OrderType() == OP_SELL && (OrderMagicNumber() == iMagicNumber)) {
            if (Ask > (OrderOpenPrice()+StopLoss*Poin)
             || Ask < (OrderOpenPrice()-TakeProfit1*Poin)
             || (TimeCurrent() > OrderOpenTime()+ (exit_time*3600)&& Ask <=OrderOpenPrice()-(exit_pips*Poin))
             ) {
               RefreshRates();
               OrderClose(OrderTicket(), OrderLots(), Ask, 3*Poin/Point, Aqua);
               }
               }
           }//end for 
...
void PrintDetails()
{
   ...
   sComment = sp;
   sComment = sComment + "OVERNIGHT EA V9a"+ NL;
   ...
   sComment = sComment + "Broker Spread = " +DoubleToStr(spread,5)  + NL;  
