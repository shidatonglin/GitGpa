...
extern int    CounterTradeMax = 7;          //Max number of Counter Trading
extern int    CounterTradeGap = 20;         //Gap of Counter Trading
extern double CounterLotMulti = 1.15;        //Multi of Counter Trading
extern int    SinglePairTP    = 40;         //TP of Single Symbol Basket
...
extern int    SignalLevelUP   = 68;         //Up Level of Confirm  SELL Signal
extern int    SignalLevelDN   = 32;         //Down Level of Confirm BUY Signal
extern string BTS = "*** Basket Trading Setting ***";
extern double BasketProfitPercent= 5;       //TP of all Trading in Percent
extern double BasketStoplossPercent = 0;   //SL of all Trading in Percent   
// % of Account Balance
...
//---Check breakeven,if profit more than BE, then setup SL to BE+5  
  ...
  {  for(i=0;i<SinglePairTradeNum;i++)
     ...
     for(i=0;i<SinglePairTradeNum;i++)
     {  if(TradeType==OP_BUY)
        {  if(OrderModify(PairTicket[i],PairOpenPrice[i],NormalizeDouble(AverageOpenPrice+5*Point,Digits),0,0,Blue)==true)
...
//---Check single pair profit. if >SinglePairTP,then close the pair  
  if(SinglePairTradeNum>0 && PairProfit>=SinglePairTP)
    CloseSpecifyOrder(Symbol(),"Single pair basket profit reach, close specified pair");
  
  iDisplayInfo("Pairprofit","Pair Profit="+AlignStr(DoubleToStr(PairProfit,2),10),3,10,40,10,TEXT_FONT,Lime);
  iDisplayInfo("PairTradeNum","Pair Order Num="+AlignStr(IntegerToString(n,0),10),3,10,55,10,TEXT_FONT,Lime);
...
/*  if(BasketProfitCash>0 && gBasketProfit>=BasketProfitCash)
  {
    {
      for(i=0; i<OrdersTotal(); i++)
      {
        if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES)==false)
        {  iError=GetLastError();
           Print("OrderSelect() Function in CheckBasketProfit error = ",iError,"--",ErrorDescription(iError));
        }
        if (OrderType()==OP_BUY)
        {
          if(OrderClose(OrderTicket(),Lots,Bid,3,Red)==true)
            Print("Basket Profit Hit, Close BUY Order OK!!!");
          else
          {  iError=GetLastError();
             Print("Basket Profit Hit, Close BUY Order Error : ",iError,"--",ErrorDescription(iError));
          }
        }
        if (OrderType()==OP_SELL)
        {
          if(OrderClose(OrderTicket(),Lots,Ask,3,Red)==true)
            Print("Basket Profit Hit, Close SELL Order OK!!!");
          else
          {  iError=GetLastError();
             Print("Basket Profit Hit, Close SELL Order Error : ",iError,"--",ErrorDescription(iError));
          }
        }
      } 
      CloseSpecifyOrder("NULL","Basket Profit Hit,Close all orders");
    }
  }
  */
  ...
  {/*
      for(i=0; i<OrdersTotal(); i++)
      {
        ...
        if (OrderType()==OP_BUY)
        {
          ...
          {  iError=GetLastError();
             Print("Basket Profit Hit, Close BUY Order Error : ",iError,"--",OrderSymbol(),"--",ErrorDescription(iError));
          }
        }
        if (OrderType()==OP_SELL)
        {
          ...
          {  iError=GetLastError();
             Print("Basket Profit Hit, Close SELL Order Error : ",iError,"--",OrderSymbol(),"--",ErrorDescription(iError));
          }
        }
      } */
