...
#define TEXT_FONT "Courier New"                             //"Consolas";"Courier New" 
...
input int             PendOrderGapPips=10;                    //Gaps of Pending placed away H/L of H4
input double          NewsTPinDollars =50;
//input double          NewsSLinDollars =0;                  //0 means SL in last Fractals
input double          RiskMoneyPercent=3;                   //Percentage equity loss for each trade,0 means fix_lots
input double          RiskRatio       =0;                    //Reward/risk ratio, 0 means use ATR to stoploss
input bool            UseMM           =true;                 //Auto calculate Lot,TP,SL
input bool            FilterUseCC     =true;                 //Use Currency Strength Filter
...
input int             CounterTradeMax =3;                    //Max number of Counter Trading
...
input bool            StrengthFilter  =false;
input ENUM_TIMEFRAMES StrengthTF      =1440;                 // TimeFrame for calculate currency strength/weakness
input double          Strength_Trigger=0;                    // Minimum % strength/weakness to trigger
     ...
     {  for(p=0;p<pairinfo[i].tradecount;p++)
        ...
        {  if(pairinfo[i].tradetype==OP_BUY)
           {  if(OrderModify(pairinfo[i].tradeticket[p],pairinfo[i].tradeopenprice[p],
              ...
              {  pairinfo[i].pairBEflag=true; Print("Ticket #",pairinfo[i].tradeticket[p],", BreakEven is hit, SL Modify OOKK!!");}
              else
...
{  int iOrders=OrdersTotal();
   ...
   {  for(i=iOrders; i>0; i--) {
        if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES)) {
          if((OrderType()<=OP_SELL) && GetMarketInfo()) {
            if(!OrderClose(OrderTicket(),OrderLots(),Price[1-OrderType()],giSlippage))   Print(notes,", error=", iError,"--",ErrorDescription(iError));
          }
   ...
   if(pair!="NULL")
   {  for(i=iOrders; i>0; i--)   {
        if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES) && (OrderSymbol() == pair))   {
          if((OrderType()<=OP_SELL) && GetMarketInfo())   {
            if(!OrderClose(OrderTicket(),OrderLots(),Price[1-OrderType()],giSlippage))  Print(notes,", error=", iError,"--",ErrorDescription(iError));
          }
        }
...
double riskmoney    =riskpercent*0.01*AccountEquity();
  ...
  {  newsBUYSL =NormalizeDouble(buystopprice-atr5,digits);
     newsSELLSL=NormalizeDouble(sellstopprice+atr5,digits);
     newsBUYTP =iHigh(pairs,PERIOD_D1,iHighest(pairs,PERIOD_D1,MODE_HIGH,10,0));
     newsSELLTP=iLow(pairs,PERIOD_D1,iLowest(pairs,PERIOD_D1,MODE_LOW,10,0));
  }
   if(UseMM==false)  {
...
bool AllowTradeTime=false; 
   ...
   {  for(k=0;k<OrdersHistoryTotal();k++)
      ...
      for(p=startshow; p<startshow+4; p++)
      {  if(   mainData[p][IMPACT]=="High"
