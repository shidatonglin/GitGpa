...
*/
  ...
  if( TotalOrdersCount() < MaxTrade ) 
  {
     ...
     {
        result=OrderSend(Symbol(),OP_BUY,mlots,Ask,Slippage,0,0,"ADX TSS - long "+DoubleToStr(mlots,2)+" on "+Symbol(),MagicNumber,0,Turquoise);
        if(result>0)
        {
         ThisBarTrade = Bars;
     ...
     if(signal_2==-1 && (lastOrderTime==0 || sqGetBarsFromOrderOpen(lastOrderTime) >= HoursBetween))
     {   
        result=OrderSend(Symbol(),OP_SELL,mlots,Bid,Slippage,0,0,"ADX TSS - short "+DoubleToStr(mlots,2)+" on "+Symbol(),MagicNumber,0,Magenta);
        if(result>0)
        {
         ThisBarTrade = Bars;
         //lastOrderTime = TimeCurrent();
