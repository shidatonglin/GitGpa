...
int start(){
  ...
  // If order existed, check for close order
  // Check for modifiy order/train stop/close order
  if(TotalOrdersCount() > 0) {
    // Check once for each bar
    if(exitTime < Time[0]){
      int exit_signal = getExitSignal();
      if(exit_signal == -1) close_buy();
      if(exit_signal == 1) close_sell();
    } else {
      exitTime = Time[0];
    }
    // Check each tick
  ...
  // If order not existed or less than max trade
  // Check for open new order
  if(TotalOrdersCount() < MaxTrade) {
    if(preTime < Time[0]){
      // New bar , check the entry critical
      int signal = getSignal();
      if(signal == 0) return(0);
      
      if(signal == 1) open_buy();
      else if(signal == -1) open_sell();
    } else {
      // Same bar, next loop
      preTime = Time[0];
      //return(0);
...
void open_buy(){
  ...
  result=OrderSend(Symbol(),OP_BUY,mlots,Ask,Slippage,0,0,"HA_SMA-long "+DoubleToStr(mlots,2)+" on "+Symbol(),MagicNumber,0,Turquoise);
  if(result>0){
    //log("IchomuA=="+IchomuA + " IchomuB==" + IchomuB + " Close[shift]==>"+Close[shift] + "ichomuTrend=="+ichomuTrend);
    double TP = 0, SL = 0;
    double SLp = 0, TPp = 0,  TSp = 0; 
    calculateStopLoss(SLp, TPp, TSp);
    if(TPp>0) TP=Ask+TPp;
    if(SLp>0) SL=Ask-SLp;
...
void open_sell(){
  int result = 0;
  double mlots = getOrderLots();
  result=OrderSend(Symbol(),OP_SELL,mlots,Bid,Slippage,0,0,"HA_SMA - short "+DoubleToStr(mlots,2)+" on "+Symbol(),MagicNumber,0,Magenta);
  if(result>0){
        ...
        if(TSp>0 && trail_mode==true){                 
          if(Bid-OrderOpenPrice()>TSp){
            if(OrderStopLoss()<Bid-TSp){
              success = OrderModify(OrderTicket(),OrderOpenPrice(),Bid-TSp,OrderTakeProfit(),0,Turquoise);
              return success;
            }
          }
        }
        ...
        if(TSp>0 && trail_mode==true){                 
          if((OrderOpenPrice()-Ask)>(TSp)){
            if((OrderStopLoss()>(Ask+TSp)) || (OrderStopLoss()==0)){
              success = OrderModify(OrderTicket(),OrderOpenPrice(),Ask+TSp,OrderTakeProfit(),0,Magenta);
              return success;
            }
          }
        }
...
void calculateStopLoss(double &SLp, double &TPp, double &TSp){
  ...
  TPp = NormalizeDouble(pf*(SLp),Digits); // 3.5 SLP
  TSp = NormalizeDouble(tf*(SLp),Digits); //0.8 SLP
