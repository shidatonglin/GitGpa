...
//|                     modified --  Kijun Sen Trailing Stop         |
//+------------------------------------------------------------------+
...
extern bool   TrailAllSymbols   = false;
...
extern int Buffer   = 20;  // buffer for kijun sen stop 
...
int start()
{
   if (!CheckTerminalStatus()) return(0); showTwoStateMessage("working","Kijun-sen trailing EA working",true);
      ...
      if (OrderType()==OP_BUY) 
      {
         ...
         double maxBuyStop = NormalizeDouble(bid-MarketInfo(OrderSymbol(),MODE_STOPLEVEL)*point,digits);
            if (OrderStopLoss()==0 && InitialStop > 0)
               ...
               {             
                  buyStop        = stopValue(OrderSymbol());     // - buffer    ??
      ...
      if (OrderType()==OP_SELL)
      {
         ...
         double minSellStop = NormalizeDouble(ask+MarketInfo(OrderSymbol(),MODE_STOPLEVEL)*point,digits);
            if (OrderStopLoss()==0 && InitialStop > 0)
               ...
               {             
                  sellStop        = stopValue(OrderSymbol()); // +buffer+spread     ????
...
{
   double ichimoku = iIchimoku(symbol,TimeFrame,Tenkan,Kijun,Senkou,MODE_KIJUNSEN,1);
...
void showTwoStateMessage(string name, string message, bool state)
{
   ...
   {   
      ...
      if (state==false)
            showMessage(message,PeachPuff);
      else  showMessage(message,PaleGreen);
   }      
}
...
void showMessage(string text, color theColor=Silver)
{
   ...
   for(i=0; i<=msg.lastMessage; i++)
   {
      ...
      ObjectSetText(name,msg.messageText[i],8,"Arial",msg.messageColor[i]);
   }
   msg.lastMessage++;
   WindowRedraw();
}