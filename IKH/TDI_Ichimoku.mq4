...
extern int    magicNum           = 201870802;
extern string comments           = "TDI_IchimokuV1.o";
...
datetime _ExpDate=0;
string label_name = "TDI_IchimokuV1";
...
int start() {
  int direction = 0;
  if(enable && Period() != PERIOD_H1) {
    log("Please attach to H1 chart!!");
    Alert("Please attach to H1 chart!!");
    enable = false;
  }
  ...
  if(TimeDay( curGmtTime ) != TimeDay( TimeGMT() )){
    log("A new day comeing!! ");
  }
  if(sqOrderOpenedThisBar(magicNum)) return(0);
  ...
  //          +"\niBarBegin--->"+iBarBegin
  //          +"\ndayStartWork--->"+ TimeToStr( dayStartWork )
  ...
  // Go long
  if(Bid > iIchimoku(NULL, PERIOD_H4 , 12 , 29 , 52 , 3 , 0) + NormalizeDouble( PipsAboveCloud * Point, Digits) 
    && Bid > iIchimoku(NULL, PERIOD_H4 , 12 , 29 , 52 , 4 , 0) + NormalizeDouble( PipsAboveCloud * Point, Digits)) {
    direction = 1;
  }

  if(Ask < iIchimoku(NULL, PERIOD_H4 , 12 , 29 , 52 , 3 , 0) - NormalizeDouble(PipsAboveCloud * Point, Digits)
    && Ask < iIchimoku(NULL, PERIOD_H4 , 12 , 29 , 52 , 4 , 0) - NormalizeDouble(PipsAboveCloud * Point, Digits)){
    direction = -1;
  }
  ...
  bool heikenLong = false;
  if(sqHeikenAshi("NULL", 0,"Low", 0) == sqHeikenAshi("NULL", 0,"Open", 0)
    && sqHeikenAshi("NULL", 0,"Close", 0) > sqHeikenAshi("NULL", 0,"Open", 0)){
    heikenLong = true;
  }

  bool heikenShort = false;
  if(sqHeikenAshi("NULL", 0,"High", 0) == sqHeikenAshi("NULL", 0,"Open", 0)
    && sqHeikenAshi("NULL", 0,"Close", 0) < sqHeikenAshi("NULL", 0,"Open", 0)){
    heikenShort = true;
  }
  int iTimeFrame = 0;
  double tdi_signal1 = iCustom(NULL,iTimeFrame,TDI_INDI,TDI_RSIPeriod,TDI_RSIPrice,TDI_VolatilityBand,TDI_RSIPriceLine,
                        TDI_RSIPriceType,TDI_TradeSignalLine,TDI_TradeSignalType,1,5);
  double tdi_signal2 = iCustom(NULL,iTimeFrame,TDI_INDI,TDI_RSIPeriod,TDI_RSIPrice,TDI_VolatilityBand,TDI_RSIPriceLine,
                        TDI_RSIPriceType,TDI_TradeSignalLine,TDI_TradeSignalType,1,5);
  double tdi_priceLine1 = iCustom(NULL,iTimeFrame,TDI_INDI,TDI_RSIPeriod,TDI_RSIPrice,TDI_VolatilityBand,TDI_RSIPriceLine,
                        TDI_RSIPriceType,TDI_TradeSignalLine,TDI_TradeSignalType,1,4);
  double tdi_priceLine2 = iCustom(NULL,iTimeFrame,TDI_INDI,TDI_RSIPeriod,TDI_RSIPrice,TDI_VolatilityBand,TDI_RSIPriceLine,
                        TDI_RSIPriceType,TDI_TradeSignalLine,TDI_TradeSignalType,1,4);

  bool tdiLong = false, tdiShort = false;
  if(tdi_priceLine2 < tdi_signal2 && tdi_priceLine1 > tdi_signal1){
    tdiLong = true;
  }
  if(tdi_priceLine2 > tdi_signal2 && tdi_priceLine1 < tdi_signal1){
    tdiShort = true;
  ...
  double mahigh = iMA( NULL, 0, 5, 2, MODE_SMA, PRICE_HIGH,1);
  double malow = iMA( NULL, 0, 5, 2, MODE_SMA, PRICE_LOW,1);
  ...
  if(getTotalOrderByType(2) > 0){
    ...
    if(cmd==OP_SELL){
      sellPrice=_bid;
      sellSL=NormalizeDouble(sellPrice+stoploss*Point,Digits);
      sellTP=NormalizeDouble(sellPrice-takeprofit*Point,Digits);
    ...
    if(cmd==OP_BUY){
      buyPrice=_ask;
      buySL = NormalizeDouble(buyPrice-stoploss*Point,Digits); //define a stop loss with interval
      buyTP=NormalizeDouble(buyPrice+takeprofit*Point,Digits);       //define a take profit
...
void log(string String)
{
   ...
   //if (!Auditing) return;
   string Filename = "logs\\" + "SecondH4BreakOut" + " (" + Symbol() + ", " + strPeriod( Period() ) + 
