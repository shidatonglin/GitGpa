...
void start() {
  ...
  for (int i=0; i<NumberOfDays; i++) {
    ...
    DrawObjects(dtTradeDate, "TargetHigh  " + TimeToStr(dtTradeDate,TIME_DATE), periodBegin, periodEnd, BoxEnd, TargetColor, BoxBreakOut_Offset,5);
    DrawObjects(dtTradeDate, "TargetLow  " + TimeToStr(dtTradeDate,TIME_DATE), periodBegin, periodEnd, BoxEnd, TargetColor, BoxBreakOut_Offset,6);
    DrawObjects(dtTradeDate, "TargetSL  " + TimeToStr(dtTradeDate,TIME_DATE), periodBegin, periodEnd, BoxEnd, StopLossColor, BoxBreakOut_Offset,7);
...
void DrawObjects(datetime dtTradeDate, string sObjName, string sTimeBegin, string sTimeEnd, string sTimeObjEnd, color cObjColor, int iOffSet, int iForm) {
  ...
  double   dPriceHigh,  dPriceLow, dPriceMid, dPriceHigh160, dPriceLow160;
  ...
  dPriceMid = dPriceHigh - dPriceLow;
  dPriceHigh160 = dPriceHigh + (dPriceMid*0.618);
  dPriceLow160 = dPriceLow - (dPriceMid*0.618);
...
//---- Upper Target
  if(iForm==5){
      ObjectSet(sObjName, OBJPROP_PRICE1, dPriceHigh160);
      ObjectSet(sObjName, OBJPROP_PRICE2, dPriceHigh160 + iOffSet*Point);
      ObjectSet(sObjName, OBJPROP_STYLE, STYLE_SOLID);
      ObjectSet(sObjName, OBJPROP_COLOR, cObjColor);
      ObjectSet(sObjName, OBJPROP_BACK, True);
   }
 ...
 //---- Lower Target 
  if(iForm==6){
      ObjectSet(sObjName, OBJPROP_PRICE1, dPriceLow160 - iOffSet*Point);
      ObjectSet(sObjName, OBJPROP_PRICE2, dPriceLow160);
      ObjectSet(sObjName, OBJPROP_STYLE, STYLE_SOLID);
      ObjectSet(sObjName, OBJPROP_COLOR, cObjColor);
      ObjectSet(sObjName, OBJPROP_BACK, True);
   }   
 //---- StopLossTarget
  if(iForm==7){
      ObjectSet(sObjName, OBJPROP_PRICE1, dPriceMid + iOffSet*Point);
      ObjectSet(sObjName, OBJPROP_PRICE2, dPriceMid - iOffSet*Point);
      ObjectSet(sObjName, OBJPROP_STYLE, STYLE_SOLID);
      ObjectSet(sObjName, OBJPROP_COLOR, cObjColor);
      ObjectSet(sObjName, OBJPROP_BACK, True);
   }
