...
extern int    magicNum           = 201870726;
extern string comments           = "Second GMT H4 V1.0";
extern int    maxSpread          = 30;
extern int     slippage          = 2;
extern string sTimeBegin         = "08:00";
//extern string boxTimeStartGMT    = "00:00";
//extern string boxTimeEndGMT     = "08:00";
...
datetime _ExpDate=0;
string label_name = "rectangle_secondH4";
...
int start() {
  ...
  double   _ask     = NormalizeDouble(MarketInfo(Symbol(), MODE_ASK), Digits); //define an upper price
	if(!secondH4Barfound){
		...
		if(barSize/_point > 350) return(0);
...
void findTargetBar(){
	...
	if(gmtHour < getHourFromTimeStr(sTimeBegin)){
		return;
	}
	datetime dtTimeBegin = StrToTime(TimeToStr(gmtTime, TIME_DATE) + " " + sTimeBegin);
  ...
  log("secondH4Barfound: iBarBegin=="+iBarBegin
     + "\n barHigh==" + barHigh
     + "\n barLow==" + barLow);
