...
extern string     TradeComment="AshFX V2";
...
extern bool       UseHorizontalLineSR=false;
extern color      SrLineColour=Turquoise;
extern int        HlAllowablePipsAway=500;
extern string     bn="Big numbers";
extern bool       UseBigNumbers=true;
extern int        BnAllowablePipsAway=1000;
...
bool SendSingleTrade(int type)
{
   ...
   //Non ECN crook
   if (!CriminalIsECN) int ticket = OrderSend(Symbol(),type, Lot, TradePrice, slippage, StopLoss, 0, TradeComment, MagicNumber,0, colour);
   ...
   if (CriminalIsECN)
   {
     ticket = OrderSend(Symbol(),type, Lot, TradePrice, slippage, 0, 0, TradeComment, MagicNumber,0, colour);
	  ...
	  {
		  if (ticket > 0)
			  bool result = OrderModify(ticket, OrderOpenPrice(),	StopLoss, 0, 0, CLR_NONE);
			  if (!result)
			  {
...
void CloseOpenTrade()
{
   ...
   RefreshRates();
   bool result = OrderClose(OriginalTradeTicketNo, OrderLots(), OrderClosePrice(), 500, CLR_NONE);
   if (result) 
   {
...
void DoesTradeNeedHedging()
{
   ...
   if (PipsLoss >= (NextHedge * Point) )
   {
      ...
      int ticket = OrderSend(Symbol(), type, SendLots, market, 50, 0, 0, hedge, MagicNumber, Magenta);
               ...
               for (int dd = OrdersTotal() - 1; dd >=0; dd--)
               {
                  ...
                  if (OrderSymbol() == Symbol() && OrderMagicNumber() == MagicNumber && OrderComment() != hedge)
                  {
                     ...
                     while (!result)
                     {
                        ...
                        {
                           result = OrderModify(OrderTicket(), OrderOpenPrice(), OrderStopLoss(), OrderOpenPrice(),0,CLR_NONE);
                           if (!result)
                           {
                              int err=GetLastError();
...
void GetHorizontalSRlines()
{
   ...
   for (int cc = ObjectsTotal() - 1; cc >= 0; cc--)
   {
      ...
      int index=StringFind(name, "Horizontal Line");
      if (index>-1) 
      {
   ...
   {
      name = ObjectName(cc);
      index=StringFind(name, "Horizontal Line");
      if (index>-1) 
      {
