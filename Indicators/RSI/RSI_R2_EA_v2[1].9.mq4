...
//| 5/18/2007 Modified to use different MAs for trend and filter     |
//|           including LSMA and HMA as custom types                 |
//+------------------------------------------------------------------+
...
extern string  Expert_Name    = "---- RSI_R2_EA_v2.9 ----";
//+---------------------------------------------------+
//|Money Management                                   |
//+---------------------------------------------------+
...
extern int     Trend_MA_Period = 200;
...
extern string  m5 = " 5 = HMA";
extern int     Trend_MA_Type = 0;
...
extern int     Trend_MA_AppliedPrice = 0;
...
extern string  ai="--MA filter settings--";
extern string  a2=" Set switch to 1 to use filter";
extern int     useFilter = 1;
extern double  Filter_MA_Period=200;
extern string  fm = "--Moving Average Types--";
extern string  fm0 = " 0 = SMA";
extern string  fm1 = " 1 = EMA";
extern string  fm2 = " 2 = SMMA";
extern string  fm3 = " 3 = LWMA";
extern string  fm4 = " 4 = LSMA";
extern string  fm5 = " 5 = HMA";
extern int     Filter_MA_Type = 5; //0=SMA, 1=EMA, 2=SMMA, 3=LWMA, 4=LSMA
extern string  fp = "--Applied Price Types--";
extern string  fp0 = " 0 = close";
extern string  fp1 = " 1 = open";
extern string  fp2 = " 2 = high";
extern string  fp3 = " 3 = low";
extern string  fp4 = " 4 = median(high+low)/2";
extern string  fp5 = " 5 = typical(high+low+close)/3";
extern string  fp6 = " 6 = weighted(high+low+close+close)/4";
extern int     Filter_MA_AppliedPrice = 0;
...
double fLSMA(int TimeFrame, int Rperiod, int prMode, int shift)
{
   int i;
   double sum, pr;
   int length;
   ...
   for(i = length; i >= 1  ; i--)
   {
     ...
     switch (prMode)
     {
     case 0: pr = iClose(NULL, TimeFrame,length-i+shift);break;
     case 1: pr = iOpen(NULL, TimeFrame,length-i+shift);break;
     case 2: pr = iHigh(NULL, TimeFrame,length-i+shift);break;
     case 3: pr = iLow(NULL, TimeFrame,length-i+shift);break;
     case 4: pr = (iHigh(NULL, TimeFrame,length-i+shift) + iLow(NULL, TimeFrame,length-i+shift))/2;break;
     case 5: pr = (iHigh(NULL, TimeFrame,length-i+shift) + iLow(NULL, TimeFrame,length-i+shift) + iClose(NULL, TimeFrame,length-i+shift))/3;break;
     case 6: pr = (iHigh(NULL, TimeFrame,length-i+shift) + iLow(NULL, TimeFrame,length-i+shift) + 2 * iClose(NULL, TimeFrame,length-i+shift))/4;break;
     }
     tmp = ( i - lengthvar)*pr;
     sum+=tmp;
    }
...
void GetMA(int TimeFrame, int MAPeriod, int MA_Type, int MA_AppliedPrice)
{
   MACurrent=iMA(NULL, TimeFrame, MAPeriod, 0, MA_Type, MA_AppliedPrice, 1);
   MAPrevious=iMA(NULL, TimeFrame, MAPeriod, 0, MA_Type, MA_AppliedPrice, 2);
}

void GetLSMA(int TimeFrame, int MAPeriod, int MA_AppliedPrice)
{
   MACurrent=fLSMA(TimeFrame, MAPeriod, MA_AppliedPrice, 1);
   MAPrevious=fLSMA(TimeFrame, MAPeriod, MA_AppliedPrice, 2);
}


void GetHMA(int TimeFrame, int MAPeriod, int MA_AppliedPrice)
{
   MACurrent=iCustom(NULL, TimeFrame, "Hma3", MAPeriod,0,MA_AppliedPrice,0,1);
   MAPrevious=iCustom(NULL, TimeFrame, "Hma3", MAPeriod,0,MA_AppliedPrice,0,2);
}

void GetMA_Values ( int TimeFrame, int Method, int myPeriod, int AppliedPrice)
{
   switch (Method)
   {
     case LSMA :  GetLSMA(TimeFrame, myPeriod, AppliedPrice);
                  break;
     case HMA  :  GetHMA(TimeFrame, myPeriod, AppliedPrice);
                  break;
     default   : GetMA( TimeFrame, myPeriod, Method, AppliedPrice);
   }
}

double GetCurrentMA(int TimeFrame, int Method, int myPeriod, int AppliedPrice)
{
  double MA;
  
  switch (Method)
  {
    case LSMA :   MA=fLSMA(TimeFrame, myPeriod,AppliedPrice,1);

    case HMA :    MA=iCustom(NULL,TimeFrame,"Hma3",myPeriod,0,AppliedPrice,0,1);

    default : MA = iMA(Symbol(),TimeFrame,myPeriod, 0, Method, AppliedPrice, 1);
  }
...
bool CheckFilter (int cmd)
{
   ...
   GetMA_Values(Filter_TimeFrame, Filter_MA_Type,  Filter_MA_Period, Filter_MA_AppliedPrice);
   filter = false;
   switch (cmd)
   {
...
bool CheckMA(int cmd)
{
  ...
  SMA_Day3 = GetCurrentMA(Signal_TimeFrame, Trend_MA_Type, Trend_MA_Period, Trend_MA_AppliedPrice);
