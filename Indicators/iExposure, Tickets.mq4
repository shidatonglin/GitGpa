...
extern bool    drawdownOnly   = true;
extern bool    showStart      = true;
...
double         highWaterMarkPipsStart;
double         lowWaterMarkPipsStart;
double         highWaterMarkCashStart;
double         lowWaterMarkCashStart;
datetime       highWaterMarkTimestampStart;
datetime       lowWaterMarkTimestampStart;
int            totalStart;
bool           firstrun = true;
...
void init()
{
   ...
   for ( int i = 0; i < OrdersTotal(); i++ )
   {
      ...
      if ( OrderType() == OP_BUY || OrderType() == OP_SELL )
      {
         ...
         if ( GlobalVariableCheck( "BaludaHWMPips" + magicNumberGV ) )
            highWaterMarkPipsStart = GlobalVariableGet( "BaludaHWMPips" + magicNumberGV );
         if ( GlobalVariableCheck( "BaludaLWMPips" + magicNumberGV ) )
            lowWaterMarkPipsStart = GlobalVariableGet( "BaludaLWMPips" + magicNumberGV );
         if ( GlobalVariableCheck( "BaludaHWMCash" + magicNumberGV ) )
            highWaterMarkCashStart = GlobalVariableGet( "BaludaHWMCash" + magicNumberGV );
         if ( GlobalVariableCheck( "BaludaLWMCash" + magicNumberGV ) )
            lowWaterMarkCashStart = GlobalVariableGet( "BaludaLWMCash" + magicNumberGV );
         if ( GlobalVariableCheck( "BaludaHWMTimestamp" + magicNumberGV ) )
            highWaterMarkTimestampStart = GlobalVariableGet( "BaludaHWMTimestamp" + magicNumberGV );
         if ( GlobalVariableCheck( "BaludaLWMTimestamp" + magicNumberGV ) )
            lowWaterMarkTimestampStart = GlobalVariableGet( "BaludaLWMTimestamp" + magicNumberGV );
         if ( GlobalVariableCheck( "BaludaTotal" + magicNumberGV ) )
            totalStart = GlobalVariableGet( "BaludaTotal" + magicNumberGV );
}
...
void start()
{
   ...
   //---- header line
   if ( ExtLines < 0 & drawdownOnly == false)
   {
...
// High-Water Mark OLD
   if ( showStart && firstrun == true)
   {
      firstrun = false;
      name = "Basket_" + almostUniqueIndex + "_HighOld_0";
      if ( ObjectFind(name) < 0 )
      {
         for ( col = 0; col < COLUMNS; col++ )
         {
            name = "Basket_" + almostUniqueIndex + "_HighOld_" + col;
            if ( ObjectCreate ( name, OBJ_LABEL, windex, 0, 0 ) )
            {
               ObjectSet ( name, OBJPROP_XDISTANCE, ExtShifts[col] );
               ObjectSet ( name, OBJPROP_YDISTANCE, ExtVertShift * ( line + 4 ) + ExtVertOffset + FooterVertOffset * 4 );
               ObjectSetText ( name, "" );
            }
         }
      }
      ...
      name = "Basket_" + almostUniqueIndex + "_HighOld_1";
      ObjectSet ( name, OBJPROP_YDISTANCE, ExtVertShift * ( line + 4 ) + ExtVertOffset + FooterVertOffset * 4 );
      ObjectSetText ( name, "" );

      name = "Basket_" + almostUniqueIndex + "_HighOld_2";
      ObjectSet ( name, OBJPROP_YDISTANCE, ExtVertShift * ( line + 4 ) + ExtVertOffset + FooterVertOffset * 4 );
      ObjectSetText ( name, "" );

      name = "Basket_" + almostUniqueIndex + "_HighOld_3";
      ObjectSet ( name, OBJPROP_YDISTANCE, ExtVertShift * ( line + 4 ) + ExtVertOffset + FooterVertOffset * 4 );
      ObjectSetText ( name, "            High-Water Start", 7, nonPropFont, ExtHeaderColor );

      name = "Basket_" + almostUniqueIndex + "_HighOld_4";
      ObjectSet ( name, OBJPROP_YDISTANCE, ExtVertShift * ( line + 4 ) + ExtVertOffset + FooterVertOffset * 4 );
      ObjectSetText ( name, "" );

      name = "Basket_" + almostUniqueIndex + "_HighOld_5";
      ObjectSet ( name, OBJPROP_YDISTANCE, ExtVertShift * ( line + 4 ) + ExtVertOffset + FooterVertOffset * 4 );
      ObjectSetText ( name, "" );
   
      background_color = ExtLossBGColor;
      if ( highWaterMarkCashStart > 0 ) background_color = ExtWinBGColor;
      name = "Basket_" + almostUniqueIndex + "_HighOld_6";
      ObjectSet ( name, OBJPROP_YDISTANCE, ExtVertShift * ( line + 4 ) + ExtVertOffset + FooterVertOffset * 4 );
      ObjectSetText ( name, RightAlign ( DoubleToStr ( highWaterMarkCashStart, 2 ), 10 ), 7, nonPropFont, White );
      name = "Basket_" + almostUniqueIndex + "_HighOld_6_background";
      DrawCell(windex, name, ExtShifts[6], ExtVertShift*(line+4)+ExtVertOffset+FooterVertOffset*4-9, 60, 13, background_color);
   
      background_color = ExtLossBGColor;
      if ( highWaterMarkPipsStart > 0 ) background_color = ExtWinBGColor;
      name = "Basket_" + almostUniqueIndex + "_HighOld_7";
      ObjectSet ( name, OBJPROP_YDISTANCE, ExtVertShift * ( line + 4 ) + ExtVertOffset + FooterVertOffset * 4 );
      ObjectSetText ( name, RightAlign ( DoubleToStr ( highWaterMarkPipsStart, 1 ), 10 ), 7, nonPropFont, White );
      name = "Basket_" + almostUniqueIndex + "_HighOld_7_background";
      DrawCell(windex, name, ExtShifts[7], ExtVertShift*(line+4)+ExtVertOffset+FooterVertOffset*4-9, 60, 13, background_color);

      name = "Basket_" + almostUniqueIndex + "_HighOld_8";
      ObjectSet ( name, OBJPROP_YDISTANCE, ExtVertShift * ( line + 4 ) + ExtVertOffset + FooterVertOffset * 4 );
      ObjectSetText ( name, "/ pair", 7, nonPropFont, ExtHeaderColor );

      background_color = ExtLossBGColor;
      if ( highWaterMarkCashStart > 0 ) background_color = ExtWinBGColor;
      name = "Basket_" + almostUniqueIndex + "_HighOld_9";
      ObjectSet ( name, OBJPROP_YDISTANCE, ExtVertShift * ( line + 4 ) + ExtVertOffset + FooterVertOffset * 4 );
      ObjectSetText ( name, RightAlign ( DoubleToStr ( highWaterMarkCashStart / MathMax(totalStart, 1), 2 ), 10 ), 7, nonPropFont, White );
      name = "Basket_" + almostUniqueIndex + "_HighOld_9_background";
      DrawCell(windex, name, ExtShifts[9], ExtVertShift*(line+4)+ExtVertOffset+FooterVertOffset*4-9, 60, 13, background_color);
   
      background_color = ExtLossBGColor;
      if ( highWaterMarkPipsStart > 0 ) background_color = ExtWinBGColor;
      name = "Basket_" + almostUniqueIndex + "_HighOld_10";
      ObjectSet ( name, OBJPROP_YDISTANCE, ExtVertShift * ( line + 4 ) + ExtVertOffset + FooterVertOffset * 4 );
      ObjectSetText ( name, RightAlign ( DoubleToStr ( highWaterMarkPipsStart / MathMax(totalStart, 1), 2 ), 10 ), 7, nonPropFont, White );
      name = "Basket_" + almostUniqueIndex + "_HighOld_10_background";
      DrawCell(windex, name, ExtShifts[10], ExtVertShift*(line+4)+ExtVertOffset+FooterVertOffset*4-9, 60, 13, background_color);
   
      objectText = "";
      if ( highWaterMarkTimestampStart > 0 )
      {
         objectText = TimeToStr( highWaterMarkTimestampStart, TIME_DATE|TIME_MINUTES );
      }
      name = "Basket_" + almostUniqueIndex + "_HighOld_11";
      ObjectSet ( name, OBJPROP_YDISTANCE, ExtVertShift * ( line + 4 ) + ExtVertOffset + FooterVertOffset * 4 );
      ObjectSetText ( name, objectText, 7, nonPropFont, ExtHeaderColor );

// Low-Water Mark Old
      name = "Basket_" + almostUniqueIndex + "_LowOld_0";
      if ( ObjectFind(name) < 0 )
      {
         for ( col = 0; col < COLUMNS; col++ )
         {
            name = "Basket_" + almostUniqueIndex + "_LowOld_" + col;
            if ( ObjectCreate ( name, OBJ_LABEL, windex, 0, 0 ) )
            {
               ObjectSet ( name, OBJPROP_XDISTANCE, ExtShifts[col] );
               ObjectSet ( name, OBJPROP_YDISTANCE, ExtVertShift * ( line + 5 ) + ExtVertOffset + FooterVertOffset * 4 );
               ObjectSetText ( name, "" );
            }
         }
      }
      ...
      name = "Basket_" + almostUniqueIndex + "_LowOld_1";
      ObjectSet ( name, OBJPROP_YDISTANCE, ExtVertShift * ( line + 5 ) + ExtVertOffset + FooterVertOffset * 4 );
      ObjectSetText ( name, "" );

      name = "Basket_" + almostUniqueIndex + "_LowOld_2";
      ObjectSet ( name, OBJPROP_YDISTANCE, ExtVertShift * ( line + 5 ) + ExtVertOffset + FooterVertOffset * 4 );
      ObjectSetText ( name, "" );

      name = "Basket_" + almostUniqueIndex + "_LowOld_3";
      ObjectSet ( name, OBJPROP_YDISTANCE, ExtVertShift * ( line + 5 ) + ExtVertOffset + FooterVertOffset * 4 );
      ObjectSetText ( name, "             Low-Water Start", 7, nonPropFont, ExtHeaderColor );

      name = "Basket_" + almostUniqueIndex + "_LowOld_4";
      ObjectSet ( name, OBJPROP_YDISTANCE, ExtVertShift * ( line + 5 ) + ExtVertOffset + FooterVertOffset * 4 );
      ObjectSetText ( name, "" );

      name = "Basket_" + almostUniqueIndex + "_LowOld_5";
      ObjectSet ( name, OBJPROP_YDISTANCE, ExtVertShift * ( line + 5 ) + ExtVertOffset + FooterVertOffset * 4 );
      ObjectSetText ( name, "" );
   
      background_color = ExtLossBGColor;
      if ( lowWaterMarkCashStart > 0 ) background_color = ExtWinBGColor;
      name = "Basket_" + almostUniqueIndex + "_LowOld_6";
      ObjectSet ( name, OBJPROP_YDISTANCE, ExtVertShift * ( line + 5 ) + ExtVertOffset + FooterVertOffset * 4 );
      ObjectSetText ( name, RightAlign ( DoubleToStr ( lowWaterMarkCashStart, 2 ), 10 ), 7, nonPropFont, White );
      name = "Basket_" + almostUniqueIndex + "_LowOld_6_background";
      DrawCell(windex, name, ExtShifts[6], ExtVertShift*(line+5)+ExtVertOffset+FooterVertOffset*4-9, 60, 13, background_color);
   
      background_color = ExtLossBGColor;
      if ( lowWaterMarkPipsStart > 0 ) background_color = ExtWinBGColor;
      name = "Basket_" + almostUniqueIndex + "_LowOld_7";
      ObjectSet ( name, OBJPROP_YDISTANCE, ExtVertShift * ( line + 5 ) + ExtVertOffset + FooterVertOffset * 4 );
      ObjectSetText ( name, RightAlign ( DoubleToStr ( lowWaterMarkPipsStart, 1 ), 10 ), 7, nonPropFont, White );
      name = "Basket_" + almostUniqueIndex + "_LowOld_7_background";
      DrawCell(windex, name, ExtShifts[7], ExtVertShift*(line+5)+ExtVertOffset+FooterVertOffset*4-9, 60, 13, background_color);

      name = "Basket_" + almostUniqueIndex + "_LowOld_8";
      ObjectSet ( name, OBJPROP_YDISTANCE, ExtVertShift * ( line + 5 ) + ExtVertOffset + FooterVertOffset * 4 );
      ObjectSetText ( name, "/ pair", 7, nonPropFont, ExtHeaderColor );

      background_color = ExtLossBGColor;
      if ( lowWaterMarkCashStart > 0 ) background_color = ExtWinBGColor;
      name = "Basket_" + almostUniqueIndex + "_LowOld_9";
      ObjectSet ( name, OBJPROP_YDISTANCE, ExtVertShift * ( line + 5 ) + ExtVertOffset + FooterVertOffset * 4 );
      ObjectSetText ( name, RightAlign ( DoubleToStr ( lowWaterMarkCashStart / MathMax(totalStart, 1), 2 ), 10 ), 7, nonPropFont, White );
      name = "Basket_" + almostUniqueIndex + "_LowOld_9_background";
      DrawCell(windex, name, ExtShifts[9], ExtVertShift*(line+5)+ExtVertOffset+FooterVertOffset*4-9, 60, 13, background_color);
   
      background_color = ExtLossBGColor;
      if ( lowWaterMarkPipsStart > 0 ) background_color = ExtWinBGColor;
      name = "Basket_" + almostUniqueIndex + "_LowOld_10";
      ObjectSet ( name, OBJPROP_YDISTANCE, ExtVertShift * ( line + 5 ) + ExtVertOffset + FooterVertOffset * 4 );
      ObjectSetText ( name, RightAlign ( DoubleToStr ( lowWaterMarkPipsStart / MathMax(totalStart, 1), 2 ), 10 ), 7, nonPropFont, White );
      name = "Basket_" + almostUniqueIndex + "_LowOld_10_background";
      DrawCell(windex, name, ExtShifts[10], ExtVertShift*(line+5)+ExtVertOffset+FooterVertOffset*4-9, 60, 13, background_color);
   
      objectText = "";
      if ( lowWaterMarkTimestampStart > 0 )
      {
         objectText = TimeToStr( lowWaterMarkTimestampStart, TIME_DATE|TIME_MINUTES );
      }
      name = "Basket_" + almostUniqueIndex + "_LowOld_11";
      ObjectSet ( name, OBJPROP_YDISTANCE, ExtVertShift * ( line + 5 ) + ExtVertOffset + FooterVertOffset * 4 );
      ObjectSetText ( name, objectText, 7, nonPropFont, ExtHeaderColor );
   }
   }
