//last modified: 07.2013 by http://forexbaron.net
//===================================================================================
//                                                                FFCal.mq4
...
extern int		OffsetHours			= -1; //default: 0
...
extern string  ah="Set to -1 for no Alert";
extern int		Alert1MinsBefore	= 30; //-1			// Set to -1 for no Alert
extern int		Alert2MinsBefore	= 5; //-1			// Set to -1 for no Alert

//added by fxdaytrader
extern bool    PopupAlerts       = true;
extern bool    SoundAlerts       = true;
extern string  AlertSoundFile    = "news.wav";
extern bool    EmailAlerts       = false;
extern bool    NotificationAlerts= false;
bool alert1done=false;
bool alert2done=false;
//

extern bool		ReportAllForUSD	= true; //false;
extern bool 	EnableLogging 		= false; 		// Perhaps remove this from externs once its working well

extern bool		ShowNextTwoEvents	= true;
extern bool		ShowVertNews		= false;//false;
...
extern string surlhi="to 99.9% you will not not have to change that:";
extern string	sUrl = "http://www.forexfactory.com/ffcal_week_this.xml";//"http://www.forexfactory.com/ffcal_week_this.xml";
string	commentStr = "FOREXFACTORY CALENDAR";
...
string GetXmlFileName()
{
   datetime tm=TimeCurrent();
	return (TimeMonth(tm) + "-" + TimeDay(tm) + "-" + TimeYear(tm) + "-" + Symbol() + Period() + "-" + "FFCal.xml");
}
...
int start()
{
   ...
   //if(DEBUG)
   //{
   //   if(NeedToGetFile) { Print("Could not find file..."); }
   //   else
   //     {
   //       Print("File found...");
   //     }
   ...
   //}
	...
	if (AllowWebUpdates)
	{
  			...
  			(TimeCurrent() - GlobalVariableGet("LastUpdateTime")) > 14400)
		{
			...
			if (DebugLevel > 0)
				Print("Wrote XML file "+xmlFileName+"... at length "+StringLen(sData)+"\n");
			...
			if (end <= 0)
			{
				...
				Print("FFCal Error - Web page download was not complete!");
				return(false);
			}
			else
			{
   ...
   //Print("Starting parse...");
	...
	if (!IsEA_Call && TimeMinute(tm) == PrevMinute)
		return (true);
	PrevMinute = TimeMinute(tm);
//	Print("FFCal NEW MINUTE...Refreshing News from XML file...");
	...
	//Print("FileSize:"+FileSize(xmlHandle));
	
	sData = FileReadString(xmlHandle, FileSize(xmlHandle));	
	while (StringLen(sData) < FileSize(xmlHandle))
	{
      int len=StringLen(sData);
	   sData = sData + FileReadString(xmlHandle, FileSize(xmlHandle)-StringLen(sData));	
      if(StringLen(sData)==len) { break; }
   }
...
//			Print(sTags[i], "  =  ", mainData[newsIdx][i]);
		...
		{   
		   ...
		   //double hours=int(StrToTime(MakeDateTime(mainData[newsIdx][DATE], mainData[newsIdx][TIME]))-g_lasttm)/60/60;
			//g_lasttm=StrToTime(MakeDateTime(mainData[newsIdx][DATE], mainData[newsIdx][TIME]));			
			//Print(mainData[newsIdx][TITLE]+" at "+StrToTime(MakeDateTime(mainData[newsIdx][DATE], mainData[newsIdx][TIME]))+" "+mainData[newsIdx][DATE]+" "+mainData[newsIdx][TIME]+" hour diff:"+hours);
         //Print(mainData[newsIdx][TITLE]+" "+MakeDateTime(mainData[newsIdx][DATE], mainData[newsIdx][TIME]));
			newsTime = StrToTime(MakeDateTime(mainData[newsIdx][DATE], mainData[newsIdx][TIME]))/*-TimeGMTOffset()*/;
			
			//Print("News time vs current:"+newsTime+" v "+TimeGMT());
			
			// Now calculate the minutes until this announcement (may be negative)
			minsTillNews = (newsTime - TimeGMTFIX()) / 60;
			if (DebugLevel > 0)
			{
			...
			{
				//added by fxdaytrader
				alert1done=false; alert2done=false;
...
//			Print("Mins till event: ", minsTillNews);
			...
			if (Alert1MinsBefore != -1 && minsTillNews == Alert1MinsBefore && !alert1done)
			  {
				doAlerts(Alert1MinsBefore+" minutes until news for "+pair+": "+mainData[newsIdx][TITLE]); //mod. by fxdaytrader
				alert1done=true;
			  }
			...
			if (Alert2MinsBefore != -1 && minsTillNews == Alert2MinsBefore && !alert2done)
			  {
				doAlerts(Alert2MinsBefore+" minutes until news for "+pair+": "+mainData[newsIdx][TITLE]); //mod. by fxdaytrader
				alert2done=true;
			  }
	...
	//Print("DEBUG1:"+dispMinutes[0]);
	//Print("DEBUG2:"+dispMinutes[1]);
...
int hSession(bool Direct)
{
	...
	if (hSession_IEType == 0)
	{
		...
		hSession_IEType = InternetOpenAFIX(InternetAgent, Internet_Open_Type_Preconfig, "0", "0", 0);
		hSession_Direct = InternetOpenAFIX(InternetAgent, Internet_Open_Type_Direct, "0", "0", 0);
...
bool GrabWeb(string strUrl, string& strWebPage)
{
	...
	hInternet = InternetOpenUrlAFIX(hSession(FALSE), strUrl, "0", 0, 
	...
	iResult = InternetReadFileFIX(hInternet, sBuffer, Buffer_LEN, lReturn);
   ...
   if (bWinInetDebug) {	Print("Closing URL web connection");   //added by MN 
   }
...
void doAlerts(string message) {
 ...
 if (EmailAlerts) SendMail(message,message);
}