...
extern bool   traderChannel      = true;
...
int start()
{
  ...
  if(TotalBuyOrders > 0 && TradeSignal > 0 && TotalBuyOrders < MaxTrades)
  {
    OrderSended = -1;
  ...
  if(TotalSellOrders > 0 && TradeSignal < 0 && TotalSellOrders < MaxTrades)
  {
    OrderSended = -1;
...
double GetLotForStep(int mNumber, int OrdCount)
{
  ...
  //StartLot = Lots_Multiple * StartLot;
  CurrLot = CurrLot * Lots_Multiple;
  return(CurrLot);
}
...
double GetStartLot()
{
  ...
  FirstLot = FirstLot * Lots_Multiple;
  return(FirstLot);
}
...
void CheckOverlapping()
{
  ...
  if (TotalBuyOrders >= 2) 
  {
    ...
    Lpos = LidingProfitOrder(MagicNumberBuy);// Get the most profit order
    Cpos = CloseProfitOrder(MagicNumberBuy);// Get the most loss order
