...
string version = "0.1";
extern int magic_1 = 101;                //Magic Number Long Cycle
extern int magic_2 = 102;                //Magic Number Short Cycle
...
double mlots_1;
double mlots_2;
double spread, floating_pl, last, previous;
int i;
...
if (nb_longs==0){
   ...
   if (RSI>PRSI) {
      OrderSend(Symbol(),OP_BUY,mlots_1,
                Ask,0,0,Ask+tp,
...
if (nb_shorts==0){
   ...
   if (RSI<PRSI) {
      OrderSend(Symbol(),OP_SELL,mlots_2,
                Bid,0,0,Bid-tp,
...
if (nb_longs>0 && nb_longs<max_trades){
   if (BUY == true){
...
if (nb_shorts>0 && nb_shorts<max_trades){
   if (SELL == true){
...
string comment = "\n \n"
        ...
        + "Entry Levels: \n \n"
	     + "Higher Buy: " + buy_max + "\n"
	     + "Lower Buy: " + buy_min + "\n"
	     + "Higher Sell: " + sell_max + "\n"
	     + "Lower Buy: " + sell_min + "\n \n"
	        
        + "Current Trades: \n \n"
        + "Longs: " + nb_longs + " / Shorts: " + nb_shorts + "\n"
...
*/       
        ...
        + "Stoplevel/Freezelevel: " + DoubleToStr(MarketInfo(Symbol(),MODE_STOPLEVEL))
	     ...
	     + "Stoploss: " + tp + " (" + DoubleToStr(tp/(10*Point),1) + " pips)";  
...
void Popup() {
   ...
   ObjectSetText(txt2, "Keops",25, "Century Gothic", LightGray);
