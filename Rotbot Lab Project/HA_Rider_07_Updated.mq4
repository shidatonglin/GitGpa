...
#include <stdlib.mqh>
//--- input parameters ---------------------------------------------------------------------------
input int      Magic_Number              = 8448; // Unique magic number per Robot Instance
...
input double   Fixed_Lot_Size            = 0.01;// Lot Size for 1st order in the chain, to Disable set to 0   
input double   Next_Lot_Factor           = 1;// Next Order Multiply Factor 
...
input double   MultiplierToDetectStopHunt = 2;// Multipler of AverageSpread to make sure no excessive spread widening
input int      TicksToCount                   = 20;// The ticks to count whilst calculating the ave spread
//--------------------------------------------
input string   Closing_Options           = "Closing Options";//Closing Options
input double   Equity_Gain_Percentage    = 0;//Equity Gain Percentage
input double   Equity_Loss_Percentage    = 0;//Equity Loss Percentage
input double   Equity_Gain_Amount        =  0;//Equity Gain Amount
input double   Equity_Loss_Amount        =  0;//Equity Loss Amount
input int      Stop_Loss_Pips            = 1000;//Stop Loss in Pips (0 to disable)
input int      Trailing_Stop_Pips        = 1000;//Trailing Stop in Pips (0 to disable)
...
input int      Max_Seconds_to_Activate   = 3600;// Max. Seconds to activate after candle starts
input int      Max_Allowed_Slippage      = 10;//Max Allowed Slippage
input string   Normal_or_ECN_Account     = "E";//N:Normal, E:ECN
//--- local parameters --------------------------------------------------------------------------
...
double         TargetSpread = 0, AverageSpread = 0, Spread = 0, SpreadTotal = 0;
int            CountedTicks = 0;
string         SpreadGvName;
...
void CheckForSpreadWidening()
{
   ...
   {
    Comment("                                   " + "PAUSED DURING A MASSIVE SPREAD EVENT. STILL MONITORING TRADES.");
...
if (!IsTesting() )
   {   
      if(CloseEnough(AverageSpread,0)) 
      {
        ...
        if (left > 0)
        {
         Comment("Calculating the average spread. "+DoubleToStr(left,0)+" left to count.");
          }     
      ...
      if(counter>=500)
      {
       ...
       Comment("Average Spread = "+DoubleToStr(AverageSpread,1));
