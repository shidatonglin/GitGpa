...
/*
   Aorit 4th, 2010, modified by sangmane: Use Pip instead of Point to make parameters broker's digits independent
*/
//----------------------- INCLUDES
#include <stdlib.mqh>
...
//+------------------------------------------------------------------+
   if(MaxLookUp>0 && MinCrossDistance>0)
   {
      ...
      Distance      = MathFloor(MathAbs((FastMACurrent-SlowMACurrent)/Pip));
...
//----------------------- AFTER PROFIT TRAILING STOP      
         case 1:
            ...
            {
               OrderModify(OrderTicket(),OrderOpenPrice(),Ask+Pip*TrailingStop,OrderTakeProfit(),0,Red);
               return(0);
            }
            }
            break;
