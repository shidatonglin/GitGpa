...
extern string s0001 = "if the price doesn't rtouches the white line, but almost touches with TouchesAddin pips then the order is iopened";//_
extern int TouchesAddin = 50; // TouchesAddin (pips 4 digits)
...
void OnTick()
{
   ...
   if ((upTrend || (noTrend && !disableNoTrend)) && OrdersTotal() < maxOrders)
   {
      ...
      if ((Ask > white_20 && prevAsk - TouchesAddin * Point <= white_20) || (Ask - TouchesAddin * Point < white_20 && prevAsk >= white_20))
      {
         // open buy trade
         if (ECN)
         {
            int ticket = OrderSend(Symbol(), OP_BUY, lot, Ask, 3, 0, 0, "Uptrend trade", Magic, 0, 0);
            OrderModify(ticket,Ask,Bid - SL*Point, Bid + TP*Point,0);
         }
         else
         {
   ...
   if ((dnTrend || (noTrend && !disableNoTrend)) && OrdersTotal() < maxOrders)
   {
      ...
      if ((Bid < white_10 && prevBid + TouchesAddin * Point >= white_10) || (Bid + TouchesAddin * Point > white_10 && prevBid <= white_10))
      {
         // open sell trade
         if (ECN)
         {
            int ticket = OrderSend(Symbol(), OP_SELL, lot, Bid, 3, 0, 0, "Dntrend trade", Magic, 0, 0);
            OrderModify(ticket,Bid,Ask+ SL*Point, Ask - TP*Point,0);
         }
         else
         {
